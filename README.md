...
- **You must use `-p 8080:80`** so that the app is reachable at http://localhost:8080. Without this port mapping, the container listens on port 80 only inside the container and nothing is served on your host.
...
### Building the image yourself (Docker or Podman)

From the project root:

```
$ docker build -t radar:latest .
# or for ARM64 (e.g. Apple Silicon):  docker build --platform linux/arm64 -t radar:latest .

$ docker run --rm -p 8080:80 radar:latest
# or:  podman run --rm -p 8080:80 radar:latest
```

Then open http://localhost:8080. Inside the container the app is served by nginx from `/opt/build-your-own-radar`; the build script copies the webpack output from `/src/build-your-own-radar/dist` to that directory.

### Uploading a CSV/JSON file into `/files/`

When running the container, you can upload a local file (CSV or JSON) and then reference it as `http://localhost:8080/files/<your-file>`.

- Upload UI: http://localhost:8080/upload
- Files listing: http://localhost:8080/files/

API spec:

- **URL**: `POST /upload`
- **Body**: `multipart/form-data` with a single field named **`file`**
- **Response**: JSON like `{ "ok": true, "path": "/files/<saved-name>" }`
- **Permissions**: uploaded file is saved with mode `655`

Example using curl:

```
curl -f -F "file=@radar.csv" http://localhost:8080/upload
```

Then open `http://localhost:8080/files/radar.csv`.

**Note on persistence:** uploads are stored inside the container filesystem by default. If you remove the container (`--rm`) or start a new container, uploaded files will be lost unless you mount a host directory:

```
podman run --rm -p 8080:80 -v /path/on/host/files:/opt/build-your-own-radar/files radar:latest
```
...
- Spawn a server that will listen on port 8080 on your host (via `-p 8080:80`).
- Mount the host directory (e.g. `/mnt/radar/files/` on Linux) into the container at `/opt/build-your-own-radar/files/`. Use any host path you prefer; the container path must be `/opt/build-your-own-radar/files`.
