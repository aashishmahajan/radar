...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (originalExt !== '.csv' && originalExt !== '.json') {
      sendJson(res, 400, { error: 'File must end with .csv or .json (the app detects format by URL extension).' })
      return
    }
    ...
    const latestName = `latest${originalExt}`
    const latestPath = path.join(FILES_DIR, latestName)
    fs.writeFileSync(latestPath, filePart.data)
    fs.chmodSync(latestPath, UPLOAD_MODE)

    sendJson(res, 201, { ok: true, path: `/files/${safeName}`, latestPath: `/files/${latestName}` })
  } catch (err) {
